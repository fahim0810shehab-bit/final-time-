...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await SeliseIAM.login(email, password);
      setUser(data);
  ...
  const handleGoogleLogin = () => {
    window.location.href = SeliseIAM.getGoogleLoginUrl();
  };
    ...
    <div className="flex h-screen w-full items-center justify-center bg-zinc-950 px-4">
      <div className="w-full max-w-md rounded-2xl bg-zinc-900 p-8 shadow-2xl border border-zinc-800">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-zinc-400">Log in to VibeBuilder to continue</p>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <span className="text-xs text-center text-zinc-500 uppercase">or</span>
          <span className="w-1/5 border-b border-zinc-700 lg:w-1/4"></span>
        </div>

        <button
          onClick={handleGoogleLogin}
          className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-sm font-semibold text-white hover:bg-zinc-700 transition-colors"
        >
